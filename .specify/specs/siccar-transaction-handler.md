# Siccar.TransactionHandler Library Specification

**Version:** 1.0
**Date:** 2025-11-12
**Status:** Proposed
**Related Constitution:** [constitution.md](../constitution.md)
**Related Specification:** [siccar-cryptography-rewrite.md](siccar-cryptography-rewrite.md)

## Executive Summary

This specification defines the requirements for creating a standalone, reusable transaction and payload management library named **Siccar.TransactionHandler**. This library will handle transaction creation, signing, verification, serialization, and multi-recipient payload encryption for the SICCAR distributed ledger platform.

## Background

### Current State Analysis

The existing transaction and payload management code is embedded within `SiccarPlatformCryptography` and has several architectural issues:
...
2. **Transaction Signing & Verification**
   - Integration with Siccar.Cryptography for signing
...
1. Cryptographic primitives (handled by Siccar.Cryptography)
...
tests/
  Siccar.TransactionHandler.Tests/
...
3. Sign hash with private key (using Siccar.Cryptography)
...
└────────────────┬────────────────────────────┬─────────┘
                 ...
                 ▼                             ▼
        ┌─────────────────┐         ┌──────────────────┐
        │Siccar.Cryptography│         │System.Text.Json│
        └─────────────────┘         └──────────────────┘
```
...
5. Builder.SignAsync(wifKey)
   ...
   5b. Double SHA-256 hash
       ↓
   5c. Sign with Siccar.Cryptography
       ↓
...
1. Create `Siccar.TransactionHandler` project
...
6. **Integration:** Successfully integrated into SICCARV3
...
| Cryptography integration issues | High | Low | Use stable Siccar.Cryptography v2.0 API |
| Incomplete backward compatibility | High | Medium | Comprehensive v1-v4 test suite |

## References

- [Current Transaction Implementation](../../src/Common/SiccarPlatformCryptography/)
- [Siccar.Cryptography Specification](siccar-cryptography-rewrite.md)
