# Siccar.RegisterService Library Specification
...
- [siccar-wallet-service.md](siccar-wallet-service.md)
- [siccar-transaction-handler.md](siccar-transaction-handler.md)

## Executive Summary

This specification defines the requirements for creating a standalone, reusable register management library named **Siccar.RegisterService**. This library will handle distributed ledger register creation, transaction storage, docket management, and data synchronization for the SICCAR distributed ledger platform. The new implementation will be architected to be portable and importable into new system architectures while maintaining compatibility with existing infrastructure.
...
```
src/
├── Siccar.RegisterService/              # Core library
│   ├── Services/
...
│
├── Siccar.RegisterService.Storage.MongoDB/   # MongoDB implementation
│   ├── RegisterRepository.cs
│   └── MongoDBConfiguration.cs
│
├── Siccar.RegisterService.Storage.InMemory/  # In-memory implementation
│   └── InMemoryRegisterRepository.cs
│
├── Siccar.RegisterService.Events.Dapr/       # Dapr event bus
│   ├── DaprEventPublisher.cs
│   └── DaprEventSubscriber.cs
│
├── Siccar.RegisterService.Events.InMemory/   # In-memory events
│   └── InMemoryEventBus.cs
│
├── Siccar.RegisterService.API/               # ASP.NET Core API
│   ├── Controllers/
...
│   └── Startup.cs
│
├── Siccar.RegisterService.Client/            # Client library
│   ├── RegisterServiceClient.cs        # HTTP client
│   └── SignalRHubClient.cs             # Real-time client
│
└── Siccar.RegisterService.Tests/             # Test projects
    ├── Unit/
    ├── Integration/
    └── Performance/
```
...
4. **Authentication**
   ...
   - Audience: `SiccarAudience` config
