...
// Payload Manager V2 Class Implementation File - The Welder/Skid Row
// 10.04.2021
		...
		public (Status result, UInt32 id, bool[]? wallets) AddPayload(byte[]? data, string[]? wallets = null, IPayloadOptions? options = null)
		{
			...
			if (wallets != null && wallets.Length > 0)
			{
				...
				for (int i = 0; i < wallets.Length; i++)
					ids[i] = WalletUtils.WalletToPubKey(wallets[i]) != null && parsed.Add(wallets[i]);
				wallets = [.. parsed];
			}
			...
			IPayload p = (Payload)ci!.Invoke([data, wallets!]);
			Status status = LastError(p);
			if (status != Status.STATUS_OK)
				return (status, 0, null);
			tx_payloads.Add(p);
			tx_modified = true;
			return (Status.STATUS_OK, (UInt32)tx_payloads.Count, ids);
		}
