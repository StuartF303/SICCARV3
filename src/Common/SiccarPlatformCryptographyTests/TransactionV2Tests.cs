// TransactionV2 Class Unit Test File - The Welder/Skid Row
// 10.04.2021
...
{
	public class TransactionV2Tests
	{
...
/* GetTxVersion() Tests */
		...
		public void GetTxVersionTest()
		{
			...
			Assert.Equal((UInt32)2, tx.GetTxVersion());
		}
...
/* GetTxTransport() Tests */
		...
		public void GetTransportTxSinglePayloadRawVersion()
		{
			...
			Assert.Equal((uint)2, BitConverter.ToUInt32(transport.Data, 0));
		...
		public void GetTransportTxMultiPayloadRawPayloadFields()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				for (int j = 0; j < c; j++)
				{
					Assert.Equal((short)0, (short)r.ReadUInt16());
		...
		public void GetTransportTxMultiPayloadRawPayloadFieldsEncrypted()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				for (int j = 0; j < c; j++)
				{
					Assert.Equal((short)2, (short)r.ReadUInt16());
		...
		public void GetTransportTxMultiPayloadSignPayloadFieldsEncryptedDiff()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				Assert.Equal(wallet2, WalletUtils.PubKeyToWallet(r.ReadBytes((int)TxDefs.SHA256HashSize), (byte)WalletNetworks.ED25519));
				Assert.True(r.ReadUInt64() > 0);
				Assert.NotEmpty(r.ReadBytes((int)WalletUtils.ReadVLSize(r)));
				Assert.Equal(sender_address, WalletUtils.PubKeyToWallet(r.ReadBytes((int)TxDefs.SHA256HashSize), (byte)WalletNetworks.ED25519));
				UInt32 c = (UInt32)WalletUtils.ReadVLSize(r);
