// PayloadManager V3 Class Unit Test File - The Welder/Skid Row
// 10.04.2021
...
{
	public class TransactionV3PManagerTests
	{
...
/* GetPayloadsInfo() Tests */
		...
		public void GetPayloadsInfoCheckSizes()
		{
			for (int i = 0; i < 10; i++)
			{
				...
				for (int j = 0; j < 10; j++)
				{
					...
					manager.AddPayload(new byte[count], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
					sizes[j] = count;
				}
				var (result, info) = tx.GetTxPayloadManager().GetPayloadsInfo();
				Assert.Equal(Status.STATUS_OK, result);
		...
		public void GetPayloadsInfoCheckHashVariedMulti()
		{
			...
			foreach (byte[] b in TestData)
				tx.GetTxPayloadManager().AddPayload(b, null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			var (result, info) = tx.GetTxPayloadManager().GetPayloadsInfo();
			Assert.Equal(Status.STATUS_OK, result);
			Assert.NotNull(info);
		...
		{
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.GetTxPayloadManager().AddPayload([0x00], ["ws1jdvlej9527m5z46lwvaq4nz6ajznz0erleys4xekrht36ucmqpcws3rcwzz"]);
			var (result, info) = tx.GetTxPayloadManager().GetPayloadsInfo();
			Assert.Equal(Status.STATUS_OK, result);
			Assert.NotNull(info);
		...
		public void GetPayloadsInfoEncryptedMultiCheck()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			for (int i = 0; i < 10; i++)
				manager.AddPayload([0x00], ["ws1jdvlej9527m5z46lwvaq4nz6ajznz0erleys4xekrht36ucmqpcws3rcwzz"]);
			var (result, info) = tx.GetTxPayloadManager().GetPayloadsInfo();
			Assert.Equal(Status.STATUS_OK, result);
			Assert.NotNull(info);
			Assert.Equal(10, info.Length);
		...
		public void GetPayloadsInfoDifferWalletPerPayload()
		{
			...
			for (int i = 0; i < wallet.Length; i++)
				manager.AddPayload([0x00], [wallet[i]]);
			var (result, info) = tx.GetTxPayloadManager().GetPayloadsInfo();
			Assert.Equal(Status.STATUS_OK, result);
			Assert.NotNull(info);
...
/* GetPayloadInfo() Tests */
		...
		public void GetPayloadInfoMixedWalletPayload()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.GetTxPayloadManager().AddPayload([0x00], [wallet1]);
			tx.GetTxPayloadManager().AddPayload([0x00]);
			tx.GetTxPayloadManager().AddPayload([0x00], [wallet2]);
			var (result, info) = tx.GetTxPayloadManager().GetPayloadInfo(1);
			Assert.NotNull(info);
			Assert.Equal(Status.STATUS_OK, result);
			Assert.True(info.GetPayloadEncrypted());
...
/* GetAllPayloads() Tests */
		...
		public void GetAllPayloadsMultiPayload()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.GetTxPayloadManager().AddPayload(TestData[0], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			tx.GetTxPayloadManager().AddPayload(TestData[1], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			var (result, info) = tx.GetTxPayloadManager().GetAllPayloads();
			Assert.Equal(Status.STATUS_OK, result);
			Assert.NotNull(info);
			Assert.Equal(TestData.Count, info.Length);
...
/* GetAccessiblePayloads() Tests */
		...
		public void GetAccessiblePayloadsMixedPayloadsNoKey()
		{
			...
			manager.AddPayload(TestData[2], [wallet2]);
		...
		public void GetAccessiblePayloadsMixedPayloadsOkaySingleKey()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			manager.AddPayload(TestData[0], wallet);
			manager.AddPayload(TestData[1], [wallet[2]]);
			manager.AddPayload(TestData[2], [wallet[0]]);
			Assert.Equal(3, (int)manager.GetPayloadsCount());
...
/* GetInaccessiblePayloads() Tests */
		...
		public void GetInaccessiblePayloadsMixedPayloads()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			manager.AddPayload(TestData[0], [wallet[1]]);
			manager.AddPayload(TestData[0], wallet);
		...
		public void GetInaccessibleAllPayloadsMixedPayloads()
		{
			...
			manager.AddPayload(TestData[0], [wallet[1]]);
			manager.AddPayload(TestData[0], wallet);
			manager.AddPayload(TestData[0], [wallet[2]]);
			var (result, info) = manager.GetInaccessiblePayloads();
			Assert.Equal(Status.STATUS_OK, result);
			Assert.NotNull(info);
...
/* GetAccessiblePayloadsData() Tests */
		...
		public void GetAccessiblePayloadsDataMixedPayloadSingleMatch()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			manager.AddPayload(TestData[0], [wallet]);
			manager.AddPayload(TestData[1], [wallet3]);
			manager.AddPayload(TestData[2], [wallet2]);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			var (result, info) = tx.GetTxPayloadManager().GetAccessiblePayloadsData(pkey);
			Assert.Equal(Status.STATUS_OK, result);
...
/* ImportPayloads() Tests */
		...
		public void ImportPayloadsOnePayloadUnencryptedNoRecipientsEmptyTx()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			manager.AddPayload([0x00], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			...
			Assert.NotNull(info);
			Assert.False(info.GetPayloadEncrypted());
			ITxFormat tx2 = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
		...
		public void ImportPayloadsMultiPayloadMultiDiffWalletsUnencryptedEmptyTx()
		{
			...
			Assert.NotNull(info);
			Assert.False(info.GetPayloadEncrypted());
			tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
...
/* AddPayload() Tests */
		...
		public void AddPayloadSingleDataUnencryptedEmptyTx()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			var (result, id, _) = manager.AddPayload([0x00], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			Assert.Equal((uint)1, id);
			Assert.Equal(Status.STATUS_OK, result);
		...
		public void AddPayloadSingleDataEncryptedEmptyTx()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			var (result, id, _) = manager.AddPayload([0x00], [wallet]);
			Assert.Equal((uint)1, id);
			Assert.Equal(Status.STATUS_OK, result);
			Assert.Equal((uint)1, manager.GetPayloadsCount());
			IPayloadInfo? info = manager.GetPayloadInfo(id).info;
		...
		public void AddPayloadMultiDataUnencrypted()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result, id, _) = manager.AddPayload(TestData[i], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
		...
		public void AddPayloadEncryptionDisabledTxPayloads()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result, id, _) = manager.AddPayload(TestData[i], wallet, new TransactionV3.PayloadOptions { EType = EncryptionType.None });
				Assert.Equal((uint)i + 1, id);
				Assert.Equal(Status.STATUS_OK, result);
				IPayloadInfo? info = manager.GetPayloadInfo(id).info;
				Assert.NotNull(info);
		...
		public void AddPayloadCompressionDisabledTxPayloads()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result, id, _) = manager.AddPayload(TestData[i], wallet, new TransactionV3.PayloadOptions { CType = CompressionType.None });
				Assert.Equal((uint)i + 1, id);
				Assert.Equal(Status.STATUS_OK, result);
				IPayloadInfo? info = manager.GetPayloadInfo(id).info;
				Assert.NotNull(info);
		...
		public void AddPayloadCompressionEncryptionDisabledTxPayloads()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result, id, _) = manager.AddPayload(TestData[i], wallet, new TransactionV3.PayloadOptions { CType = CompressionType.None, EType = EncryptionType.None });
				Assert.Equal((uint)i + 1, id);
				Assert.Equal(Status.STATUS_OK, result);
				IPayloadInfo? info = manager.GetPayloadInfo(id).info;
				Assert.NotNull(info);
		...
		public void AddPayloadEncryptionDisabledTxPayloadsSigned()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			for (int i = 0; i < TestData.Count; i++)
				Assert.Equal(Status.STATUS_OK, manager.AddPayload(TestData[i], wallet, new TransactionV3.PayloadOptions { EType = EncryptionType.None }).result);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			for (int i = 0; i < TestData.Count; i++)
			{
				IPayloadInfo? info = manager.GetPayloadInfo((uint)i + 1).info;
		...
		public void AddPayloadCompressionDisabledTxPayloadsSigned()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			for (int i = 0; i < TestData.Count; i++)
				Assert.Equal(Status.STATUS_OK, manager.AddPayload(TestData[i], wallet, new TransactionV3.PayloadOptions { CType = CompressionType.None }).result);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			for (int i = 0; i < TestData.Count; i++)
			{
				IPayloadInfo? info = manager.GetPayloadInfo((uint)i + 1).info;
		...
		public void AddPayloadCompressionEncryptionDisabledTxPayloadsSigned()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			for (int i = 0; i < TestData.Count; i++)
				Assert.Equal(Status.STATUS_OK, manager.AddPayload(TestData[i], wallet, new TransactionV3.PayloadOptions { CType = CompressionType.None, EType = EncryptionType.None }).result);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			for (int i = 0; i < TestData.Count; i++)
			{
				IPayloadInfo? info = manager.GetPayloadInfo((uint)i + 1).info;
		...
		public void AddPayloadMultiDataUnencryptedRebuildSingle()
		{
			...
			Assert.NotNull(transaction);
			manager = transaction.GetTxPayloadManager();
			var (result2, id2, _) = manager.AddPayload(TestData[0], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
...
/* GetPayloadData() Tests */
		...
		public void GetPayloadDataValidIdEncrypted()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			var (result, _, _) = manager.AddPayload([0x00], [wallet]);
			Assert.Equal(Status.STATUS_OK, result);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			var (result2, data) = tx.GetTxPayloadManager().GetPayloadData(1);
			Assert.Null(data);
		...
		public void GetPayloadDataUnencryptedWithKey()
		{
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			var (result, _, _) = manager.AddPayload([0x00], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			Assert.Equal(Status.STATUS_OK, result);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
		...
		public void GetPayloadDataMultiPayloadDataUnencryptedWithKey()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result, _, _) = manager.AddPayload(TestData[i], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
				Assert.Equal(Status.STATUS_OK, result);
			}
			Assert.Equal((uint)TestData.Count, tx.GetTxPayloadManager().GetPayloadsCount());
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
		...
		public void GetPayloadDataMultiPayloadEncryptedWithKey()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result, _, _) = manager.AddPayload(TestData[i], [wallet]);
				Assert.Equal(Status.STATUS_OK, result);
			}
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			Assert.Equal((uint)TestData.Count, tx.GetTxPayloadManager().GetPayloadsCount());
...
/* ReplacePayloadData() Tests */
		...
		public void ReplacePayloadDataSinglePayloadUnencrypted()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			var (result2, id, _) = manager.AddPayload([0x00], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
			Assert.Equal(Status.STATUS_OK, result2);
		...
		public void ReplacePayloadDataSinglePayloadEncrypted()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			IPayloadManager manager = tx.GetTxPayloadManager();
			var (result2, id, _) = manager.AddPayload([0x00], wallet);
			Assert.Equal(Status.STATUS_OK, result2);
			var pi = manager.GetPayloadInfo(id);
			Assert.NotNull(pi.info);
		...
		public void ReplacePayloadDataMultiPayloadUnencrypted()
		{
			...
			for (int i = 0; i < TestData.Count; i++)
			{
				var (result2, id, _) = manager.AddPayload(TestData[i], null, new TransactionV3.PayloadOptions { CType = CompressionType.None });
				Assert.Equal(Status.STATUS_OK, result2);
		...
		public void ReplacePayloadDataMultiPayloadEncryptedResetSigned()
		{
			...
			Assert.Equal(Status.STATUS_OK, manager.ReplacePayloadData(2, TestData[0]));
...
/* AddPayloadWallets() Tests */
		...
		public void AddPayloadWalletsLowId()
		{
			string wallet = "ws1jdvlej9527m5z46lwvaq4nz6ajznz0erleys4xekrht36ucmqpcws3rcwzz";
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			var (result, _, _) = tx.GetTxPayloadManager().AddPayload([0x00]);
			Assert.Equal(Status.STATUS_OK, result);
			var status = tx.GetTxPayloadManager().AddPayloadWallets(0, [wallet]);
		...
		public void AddPayloadWalletsHighId()
		{
			...
			var (result, _, _) = tx.GetTxPayloadManager().AddPayload([0x00]);
			Assert.Equal(Status.STATUS_OK, result);
			var status = tx.GetTxPayloadManager().AddPayloadWallets(2, [wallet]);
			Assert.Equal(Status.TX_BAD_PAYLOAD_ID, status.result);
			Assert.Null(status.wallets);
		}
		[Fact]
		...
		public void AddPayloadWalletsSingleEncryptedNoKey()
		{
			...
			var (result, _, _) = manager.AddPayload([0x00], [wallet]);
			Assert.Equal(Status.STATUS_OK, result);
			var status = manager.AddPayloadWallets(1, [wallet2], null);
		...
		public void AddPayloadWalletsSingleEncryptedBadKey()
		{
			...
			var (result, _, _) = manager.AddPayload([0x00], [wallet]);
			Assert.Equal(Status.STATUS_OK, result);
			var status = manager.AddPayloadWallets(1, [wallet2], "hello");
...
/* RemovePayloadWallets() Tests */
		...
		public void RemovePayloadWalletsHighId()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.GetTxPayloadManager().AddPayload([0x00]);
			var (result, wallets) = tx.GetTxPayloadManager().RemovePayloadWallets(2, [wallet]);
			Assert.Equal(Status.TX_BAD_PAYLOAD_ID, result);
			Assert.Null(wallets);
		}
		[Fact]
