...
// TransactionV4 Class Unit Test File - The Welder/Skid Row
// 10.04.2021
...
{
	public class TransactionV4Tests
	{
		...
		public TransactionV4Tests()
		{
			sender_address = WalletUtils.PubKeyToWallet(module.CalculatePublicKey(WalletUtils.WIFToPrivKey(sender_pkey)!.Value.PrivateKey).pubkey,
...
/* GetTxSender() Tests */
		...
		public void GetTxSenderModifiedTxReplacePayloadData()
		{
			...
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			Assert.Equal(sender_address, tx.GetTxSender().sender);
			tx.GetTxPayloadManager().ReplacePayloadData(1, new byte[] { 0x00 });
			var (result, sender) = tx.GetTxSender();
			Assert.Equal(Status.TX_NOT_SIGNED, result);
			Assert.Null(sender);
		}
...
/* GetTxRecipients() Tests */
		...
		{
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.SetTxRecipients(new string[] { sender_address! });
			(Status status, string[] recipients) = tx.GetTxRecipients();
			Assert.Equal(Status.STATUS_OK, status);
...
/* SetTxRecipients() Tests */
		...
		{
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			(Status status, bool[]? recipients) = tx.SetTxRecipients(Array.Empty<string>());
			Assert.Equal(Status.STATUS_OK, status);
		...
		public void SetTxRecipientsSingleValid()
		{
			...
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			(Status status, bool[]? recipients) = tx.SetTxRecipients(new[] { wallet });
			Assert.Equal(Status.STATUS_OK, status);
			Assert.NotNull(recipients);
			Assert.Single(recipients);
		...
		public void SetTxRecipientsOneRecipientSameReplace()
		{
			...
			Assert.Single(r);
			Assert.Equal(wallet, r[0]);
			(Status status2, bool[]? recipients2) = tx.SetTxRecipients(new[] { wallet });
			Assert.Equal(Status.STATUS_OK, status2);
		...
		public void SetTxRecipientsOneRecipientDifferentReplace()
		{
			...
			Assert.Single(r);
			Assert.Equal(wallet, r[0]);
			(Status status2, bool[]? recipients2) = tx.SetTxRecipients(new[] { wallet2 });
			Assert.Equal(Status.STATUS_OK, status2);
			Assert.NotNull(recipients2);
			Assert.Single(recipients2);
			Assert.True(recipients2[0]);
		...
		public void SetTxRecipientsResetRecipients()
		{
			...
			for (int i = 0; i < wallet.Length; i++)
				Assert.Equal(wallet[i], r[i]);
			(Status status2, bool[]? recipients2) = tx.SetTxRecipients(Array.Empty<string>());
			Assert.Equal(Status.STATUS_OK, status2);
			Assert.NotNull(recipients2);
...
/* SetPrevTxHash() Tests */
		...
		public void SetPrevTxHashSignedSamePrTx()
		{
			...
			Assert.NotNull(transport);
			Assert.NotNull(transport.TxId);
			Assert.NotEmpty(transport.TxId);
		}
		[Fact]
...
/* SignTx() Tests */
		...
		[Fact]
		public void SignTxUnInitSM()
		{
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.GetTxPayloadManager().AddPayload(new byte[] { 0x00 });
			Assert.Equal(Status.TX_SIGNING_FAILURE, tx.SignTx(sender_pkey));
		}
		[Fact]
		...
		public void SignTxModified()
		{
			...
			Assert.Null(send);
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			Assert.Equal(Status.STATUS_OK, manager.ReplacePayloadData(1, new byte[] { 0x00 }));
			sig = tx.GetTxSignature().signature;
			hash = tx.GetTxHash().hash;
			ts = tx.GetTxTimeStamp().timestamp;
			send = tx.GetTxSender().sender;
		...
		public void SignTxPayloadWalletsReleasePayloads()
		{
			...
			IPayloadManager manager = tx.GetTxPayloadManager();
			manager.AddPayload(new byte[] { 0x00 }, wallet);
			manager.AddPayload(new byte[] { 0x00 }, new string[] { wallet2 });
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
...
/* VerifyTx() Tests */
		...
		[Fact]
		public void VerifyTxNullVerifySM()
		{
			ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
			tx.GetTxPayloadManager().AddPayload(new byte[] { 0x00 });
			Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
			Assert.Equal(Status.TX_VERIFY_FAILURE, tx.VerifyTx());
		}
		[Fact]
...
/* GetTxTransport() Tests */
		...
		public void GetTransportTxSinglePayloadRawMultiRecipients()
		{
			...
			for (int i = 0; i < c; i++)
				Assert.Equal(wallet[i], WalletUtils.PubKeyToWallet(WalletUtils.ReadVLArray(r), SizeFields.WalletNetwork));
			Assert.Null(transport.RegisterId);
			Assert.Null(transport.TxId);
		}
		[Fact]
		...
		public void GetTransportTxMultiPayloadRawPayloadFields()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				for (int j = 0; j < count; j++)
				{
					UInt32 id = tx.GetTxPayloadManager().AddPayload(new byte[] { 0x00 }).id;
		...
		public void GetTransportTxMultiPayloadRawPayloadFieldsEncrypted()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				Assert.Equal((uint)wallet.Length, c);
				for (int j = 0; j < c; j++)
					Assert.Equal(wallet[j], WalletUtils.PubKeyToWallet(WalletUtils.ReadVLArray(r), SizeFields.WalletNetwork));
				r.ReadUInt64();
				WalletUtils.ReadVLArray(r);
				...
				for (int j = 0; j < c; j++)
				{
					...
					for (int k = 0; k < p; k++)
					{
						...
						Assert.NotEmpty(hs);
						Assert.Equal((int)SizeFields.SHA256HashSize, hs.Length);
		...
		public void GetTransportTxMultiPayloadSignedPayloadFields()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				WalletUtils.ReadVLSize(r);
				Assert.NotEmpty(WalletUtils.ReadVLArray(r));
				Assert.Equal(sender_address, WalletUtils.PubKeyToWallet(WalletUtils.ReadVLArray(r), SizeFields.WalletNetwork));
				UInt32 c = (UInt32)WalletUtils.ReadVLSize(r);
				Assert.Equal((uint)count, c);
				for (int j = 0; j < c; j++)
				{
		...
		public void GetTransportTxMultiPayloadSignPayloadFieldsEncryptedDiff()
		{
			...
			{
				ITxFormat tx = TransactionBuilder.Build(TransactionVersion.TX_VERSION_3);
				tx.SetTxRecipients(new string[] { wallet2 });
				int count = rnd.Next(1, 10);
				for (int j = 0; j < count; j++)
					tx.GetTxPayloadManager().AddPayload(new byte[] { 0x00 }, wallet);
				Assert.Equal(Status.STATUS_OK, tx.SignTx(sender_pkey));
				...
				WalletUtils.ReadVLSize(r);
				Assert.Equal(wallet2, WalletUtils.PubKeyToWallet(WalletUtils.ReadVLArray(r), SizeFields.WalletNetwork));
				Assert.True(r.ReadUInt64() > 0);
				WalletUtils.ReadVLArray(r);
				WalletUtils.ReadVLSize(r);
				Assert.NotEmpty(WalletUtils.ReadVLArray(r));
				...
				for (int j = 0; j < c; j++)
				{
					...
					{
						WalletUtils.ReadVLSize(r);
						Assert.Equal(wallet[k], WalletUtils.PubKeyToWallet(WalletUtils.ReadVLArray(r), SizeFields.WalletNetwork));
						Assert.NotEmpty(WalletUtils.ReadVLArray(r));
					}
					Assert.NotEmpty(WalletUtils.ReadVLArray(r));
					Assert.NotEmpty(WalletUtils.ReadVLArray(r));
		...
		public void GetTransportTxMultiPayloadSignedRebuild()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				Assert.NotNull(transport2.TxId);
				Assert.Equal(transport.TxId, transport2.TxId);
			}
		}
		[Fact]
