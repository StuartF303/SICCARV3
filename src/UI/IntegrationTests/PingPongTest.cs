...
namespace Siccar.IntegrationTests
{
    public class PingPongTest : IDisposable
    {
        ...
        private int _publishBlueprintRetryCount;
        ...
        public async Task<string> SetupTest()
        {
            ...
            TransactionModel? bpTxId = null;
         
            bpTxId = await PublishBlueprint(blueprint);
        ...
        private async Task<TransactionModel?> PublishBlueprint(Blueprint? blueprint)
        {
            TransactionModel? bpTxId = null;
            try
            {
                bpTxId = await _blueprintServiceClient.PublishBlueprint(pingWallet, registerId, blueprint);
            }
            catch (HttpStatusException e) when (e.Message.Contains("User Tenant is not authorized to access register"))
            {
                if (_publishBlueprintRetryCount < 10)
                {
                    Console.WriteLine("Waiting for register to be added to tenant, retrying...");
                    _publishBlueprintRetryCount += 1;
                    Thread.Sleep(500);
                    await PublishBlueprint(blueprint);
                }
                else
                {
                    Console.WriteLine(e);
