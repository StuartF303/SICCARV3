...
using IntegrationTests.MicrosoftHackathon;
    ...
    class Program
    {
        ...
        static async Task<int> Main(string[] args)
        {
            ...
            else
            {
                ...
                if (bool.Parse(_configuration["UseClientCredentialFlow"]))
                {
            ...
            Console.WriteLine("Press 1 for normal tests, press 2 for hackathon data import");

            var input = int.Parse(Console.ReadLine());
            if (input == 2)
            {
                var importTest = new HackathonImport(_serviceProvider, bearer);
                await importTest.SetupTest();

                await importTest.Import();
